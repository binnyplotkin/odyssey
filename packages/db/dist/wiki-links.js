...
export function parseWikilinks(body) {
    ...
    while ((m = WIKILINK_RE.exec(body)) !== null) {
        const rawSlug = (_a = m[1]) === null || _a === void 0 ? void 0 : _a.trim();
        if (!rawSlug)
            continue;
        const display = (_c = (_b = m[2]) === null || _b === void 0 ? void 0 : _b.trim()) !== null && _c !== void 0 ? _c : null;
        out.push({
            raw: m[0],
            slug: rawSlug,
            display,
...
export function extractReferencedSlugs(body) {
    const seen = new Set();
    const out = [];
...
export function formatWikilink(slug, display) {
 ...
 */
export function resolveWikilinks(body, titleBySlug) {
    if (!body)
        return body;
    return body.replace(WIKILINK_RE, (match, rawSlug, rawDisplay) => {
        var _a, _b;
        const slug = rawSlug.trim();
        const knownTitle = titleBySlug.get(slug);
        if (!knownTitle) {
            // Unknown target — preserve the author's display (or slug) but mark "?"
            const d = (_a = rawDisplay === null || rawDisplay === void 0 ? void 0 : rawDisplay.trim()) !== null && _a !== void 0 ? _a : slug;
            return `[[${slug}|${d} ?]]`;
        }
        const display = (_b = rawDisplay === null || rawDisplay === void 0 ? void 0 : rawDisplay.trim()) !== null && _b !== void 0 ? _b : knownTitle;
        return `[[${slug}|${display}]]`;
        void match;
    });
}
 ...
 */
export function flattenWikilinks(body, titleBySlug) {
    if (!body)
        return body;
    return body.replace(WIKILINK_RE, (_match, rawSlug, rawDisplay) => {
        const slug = rawSlug.trim();
        const display = rawDisplay === null || rawDisplay === void 0 ? void 0 : rawDisplay.trim();
        if (display)
            return display;
        const known = titleBySlug === null || titleBySlug === void 0 ? void 0 : titleBySlug.get(slug);
        if (known)
            return known;
