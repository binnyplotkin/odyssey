...
function OceanField({
  ...
  useFrame((_, delta) => {
    ...
    const rawSignal = micOn ? clamp01(liveAudio.energy * 0.9 + liveAudio.peak * 0.68) : 0;
    if (micOn) {
    ...
    const detectedTarget = micOn ? smoothstep(0.012, 0.115, gatedSignal + liveAudio.mid * 0.06 + liveAudio.bass * 0.04) : 0;
    const loudTarget = micOn ? smoothstep(0.03, 0.32, gatedSignal * 2.0 + liveAudio.peak * 0.34) : 0;
    const voiceTarget = micOn ? smoothstep(0.014, 0.18, gatedSignal + liveAudio.peak * 0.12 + liveAudio.energy * 0.08) : 0;
    ...
    const volumeTarget = micOn
      ? smoothstep(0.01, 0.34, gatedSignal * 2.35 + liveAudio.peak * 0.26 + liveAudio.energy * 0.12)
      : 0;
    m.volume += (volumeTarget - m.volume) * (volumeTarget > m.volume ? 0.09 : 0.035);
    const dbSignal = micOn
      ? Math.max(1e-4, gatedSignal * 1.2 + liveAudio.energy * 0.18 + liveAudio.peak * 0.08)
      : 1e-4;
    ...
    const audioPresenceTarget = micOn
      ? smoothstep(0.026, 0.22, gatedSignal + liveAudio.mid * 0.04 + liveAudio.bass * 0.03 + liveAudio.peak * 0.06)
      : 0;
    audioGate.presence +=
    ...
    s.energy += (((micOn ? liveAudio.energy : 0) * activityGate) - s.energy) * 0.12;
    s.bass += (((micOn ? liveAudio.bass : 0) * activityGate) - s.bass) * 0.1;
    s.mid += (((micOn ? liveAudio.mid : 0) * activityGate) - s.mid) * 0.11;
    s.high += (((micOn ? liveAudio.high : 0) * activityGate) - s.high) * 0.12;
    s.peak += (((micOn ? liveAudio.peak : 0) * activityGate) - s.peak) * 0.15;
