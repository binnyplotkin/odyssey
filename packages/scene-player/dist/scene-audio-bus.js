...
export class SceneAudioBus {
    constructor() {
        ...
        this.voiceGain = null;
        this.nextVoiceStartTime = 0;
        this.scheduledVoiceSources = new Set();
     ...
     */
    enqueueVoiceFrame(samples, sampleRate) {
     ...
     */
    setAmbience(trackId) {
        if (trackId === this.currentAmbienceId)
            return;
        this.currentAmbienceId = trackId;
        this.fadeOutCurrentAmbience();
        if (trackId)
            this.fadeInNewAmbience(trackId);
    }
    fadeOutCurrentAmbience() {
        ...
        const interval = window.setInterval(() => {
            ...
            if (step >= steps) {
                ...
                el.pause();
                el.src = "";
                if (this.ambienceEl === el)
                    this.ambienceEl = null;
            }
        }, stepMs);
    }
    fadeInNewAmbience(trackId) {
     ...
     */
    static decodeFloat32Base64(base64) {
        try {
